...
| `artifacts/whale_noise_report_latest.json`, `scoreboard_whale_noise.json` | Make-on-whale / take-on-noise report (fill rate, toxicity, combined-vs-legs verdict, ex-post whale-flow EV) + scoreboard from `apps.measure_whale_noise` (see `docs/WHALE_NOISE_RUNBOOK.md`) |
...
### Make-on-whale, take-on-noise (Kalshi tennis)

Three paper legs replay one event stream of polled L2 books and public trade
prints from the Kalshi tennis match series: when a **whale-class** print lifts
one side (≥1,000 contracts, ≥$500, ≥10× the market's recent median — the public
tape is anonymous, so a whale is a size class, never a name), the maker leg rests
a same-direction bid **one tick behind** the pre-impact touch, inventory-capped;
when only retail longshot flow (<20¢, no whale in the last 120 s) is hitting, the
taker leg fades the favourite at the touch. `kalshi_whale_noise_combined` runs
both in one ledger; `kalshi_whale_maker_leg` / `kalshi_noise_taker_leg` are the
single-leg controls. Maker fills use an explicit **conservative queue model**
(behind every displayed contract, later books only lengthen the queue, fills only
from later prints, trade-through capped at the printed size, no fills without a
forward tape). Pre-registered pass: combined paper PnL > either leg alone; fill
rate, markout toxicity and whale follow-through are logged on every run, and an
ex-post check on settled tennis markets asks whether the whale size class is +EV.

```bash
uv run python -m apps.measure_whale_noise                                    # fixtures, no network
uv run python -m apps.book_logger --series KXATPMATCH KXWTAMATCH --interval 15  # capture a forward tape ...
uv run python -m apps.measure_whale_noise --archive artifacts/books          # ... and replay it (measures fills)
uv run python -m apps.measure_whale_noise --network --kalshi-env prod --harvest-trades   # snapshot + ex-post
```

Same caps as the FLB tracks ($25/order, $75/market, $75 daily, $1,000 collateral),
Kalshi fee schedule, conservative marks. Measured results, the fill model and its
limits: `docs/WHALE_NOISE_RUNBOOK.md`.
...
airport station, one local date, NOAA WRH hourly table) and the same station's
free public METAR/ASOS history (aviationweather.gov, NWS fallback; no keys). A
bucket entirely below the hourly running high is dead at any hour; after 17:00
local with the temperature ≥ 2°F (1°C) below the high and not rising, buckets
more than 1° above the all-report high are dead and the bucket covering the
plausible range is the certain YES; once the next local day's first observation
exists the final high decides every leg. Dead legs are bought as NO (certain
legs as YES) at the touch when the ask leaves ≥ 2¢ net after the 5 % weather
taker fee, $25/order, 75/market. Every ambiguity refuses: unparseable station,
unknown timezone, missing / stale / mismatched observations, rounding ties
across a bucket edge, too early, not falling, no ask.

```bash
uv run python -m apps.measure_weather                     # fixture replay: 4 steps, 32 positions, every branch
uv run python -m apps.measure_weather --network           # public Gamma + CLOB + aviationweather.gov, one cycle
```
...
uv run python -m apps.measure_whale_noise --help    # whale-follow maker + noise fade legs, archive replay (see docs/WHALE_NOISE_RUNBOOK.md)
...
research/      scoreboard (13 isolated tracks over shared snapshots), Polymarket arb tracks, artifact + gate-report writers, harvest analysis, news signal stubs,
               ...
               tourist_fade (tape replay with settlement, markouts, live track runner), whale_noise (tape replay, 3 legs, toxicity, ex-post whale EV)
apps/          measure_all, measure_polymarket_arb, measure_flb, measure_tennis_basis, measure_weather, measure_weather_buckets, measure_weather_calibration, measure_tourist_fade, measure_whale_noise, book_logger, paper_loop, paper_runner, dashboard_api
dashboard/     Vite + React static scoreboard reading public/artifacts/*.json
docs/          ASSUMPTIONS.md audit, NEWS_UNDERREACTION.md lane audit, POLYMARKET_ARB.md runbook + findings, RUNBOOK_gated_cross_venue.md, FLB_RUNBOOK.md, TENNIS_BASIS.md pre-registration + audit, SPECIALIST_SCOREBOARD.md pre-registration + audit, WEATHER_DEAD_BUCKET.md, WEATHER_BUCKETS.md, WEATHER_CALIBRATION.md, FADE_THE_TOURIST.md, WHALE_NOISE_RUNBOOK.md, BOOK_LOGGER.md
```
...
| `kalshi_whale_noise_combined` (standalone, `apps.measure_whale_noise`) | whale-class print (≥1,000 ct, ≥$500, ≥10× median) → bid one tick behind, conservative queue fills; retail longshot print with no whale in 120 s → taker fade; same caps | the make-on-whale / take-on-noise experiment; pass = beats both single legs |
| `kalshi_whale_maker_leg`, `kalshi_noise_taker_leg` | one leg each, same event stream | controls for the combined track; fill rate + markout toxicity logged |
